...
## 3. Roadmap Futuro — Sprints 8 a 11
...
**Entregáveis:**
1. Script `sprint8_xgboost.py` — Pipeline completo
2. Modelo serializado `modelo_sinistralidade.pkl` + `modelo_risco_produto.pkl`
3. Métricas: MAE, RMSE, R² (Modelo 1); Accuracy, F1, AUC (Modelo 2)
4. SHAP values salvos para visualização no dashboard
5. Nova página "Predição" no dashboard com projeções e explicabilidade

**Riscos e Mitigações:**

| Risco | Impacto | Mitigação |
|---|---|---|
| Poucos dados (132 reg. Modelo 1) | Overfitting | Regularização forte + cross-validation temporal |
| Mudança estrutural (fusão Hapvida-NDI) | Quebra de padrão | Flag binária + tratamento de outliers |
| Dados estáticos do SIB (snapshot) | Modelo 2 não evolui | Pipeline de atualização mensal (Sprint 10) |
| Interpretabilidade para stakeholders | Rejeição do modelo | SHAP waterfall + narrativa em linguagem natural |

**Estimativa:** ~50 minutos de implementação autônoma.
