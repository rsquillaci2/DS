...
## 3. Sprints Concluídas — Fase 2

### Sprint 9 — MCP Server (Motor como Serviço para Agentes IA) ✅

**Objetivo:** Expor o Motor de Sinistralidade como servidor MCP (Model Context Protocol), permitindo que agentes de IA consultem dados em linguagem natural.

**Stack técnico:**

| Componente | Tecnologia | Descrição |
|-----------|-----------|----------|
| Protocolo | MCP (JSON-RPC via stdio) | Padrão aberto para integração com LLMs |
| SDK | mcp[cli] Python | Servidor FastMCP |
| Banco | DuckDB (read-only) | Consultas diretas ao banco analítico |
| Modelo ML | joblib | Carregamento do XGBoost serializado |
| Compatibilidade | Claude Desktop, Manus | Qualquer agente com suporte MCP |

**12 Tools disponíveis:**

| Tool | Função |
|------|--------|
| `buscar_operadora` | Busca por nome ou código ANS |
| `ficha_operadora` | Ficha completa (financeiro + benchmark + vidas + predição) |
| `ranking_sinistralidade` | Top N maior/menor sinistralidade |
| `ranking_receita` | Top N por receita |
| `benchmark_modalidade` | Referências por tipo de operadora |
| `serie_temporal` | Histórico trimestral (24 trimestres) |
| `evolucao_beneficiarios` | Evolução mensal de vidas |
| `predicao_operadora` | Predição XGBoost próximo trimestre |
| `comparar_operadoras` | Comparação lado a lado (até 10) |
| `distribuicao_mercado` | Visão geral do mercado |
| `operadoras_criticas` | Operadoras com sinistralidade >100% |
| `sql_livre` | Consulta SQL customizada (read-only) |

**Deliverables:**
1. `mcp_server.py` — Servidor MCP completo (12 tools, ~400 linhas)
2. `MCP_SERVER_README.md` — Documentação de uso e configuração
3. Testes unitários validados (12/12 tools OK)

---

### Sprint 10 — Pipeline de Atualização Automática ✅

**Objetivo:** Manter o motor atualizado automaticamente, baixando dados novos da ANS, executando ETL, recalculando benchmarks e retreinando o modelo ML sem intervenção manual.

**Arquitetura do Pipeline:**

```
┌─────────────────────────────────────────────────────────────┐
│  CRON (Trimestral: 1º Jan/Abr/Jul/Out 03:00)        │
│  CRON (Mensal: Dia 15 às 06:00 — verificação)       │
└─────────────────────────────────────────────────────────────┘
                          │
              ┌───────────▼───────────┐
              ...
              └─────┬───────┬───────┘
                    │       │
         ┌───────▼──┐  ┌─▼──────────┐
         │ download │  │ etl_refresh │
         │ _ans.py  │  │    .py      │
         └────┬────┘  └─┬────┬─────┘
              │         │    │
         FTP ANS    ETL   ML Retrain
```
...
|-----------|---------|------|
| Trimestral | 1º dia Jan/Abr/Jul/Out às 03:00 | Pipeline completo (download + ETL + ML) |
| Mensal | Dia 15 às 06:00 | Verificação de novos dados (download-only) |

**Fontes de dados (FTP ANS):**

| Fonte | URL | Frequência de Publicação |
|-------|-----|---------------------------|
| DIOPS | `dadosabertos.ans.gov.br/FTP/PDA/demonstracoes_contabeis/` | Trimestral (~3 meses após fim do tri) |
| SIB | `dadosabertos.ans.gov.br/FTP/PDA/informacoes_consolidadas/` | Mensal (~2 meses de atraso) |

**Teste end-to-end:**
- Pipeline executado com sucesso em **7 segundos** (modo ETL-only)
- 849 operadoras processadas, 18.739 registros históricos, 186 predições geradas
- Modelo retreinado: R²=0.74, CV 5-fold=0.72 ± 0.02

**Deliverables:**
1. `pipeline/download_ans.py` — Download automático DIOPS + SIB do FTP ANS
2. `pipeline/etl_refresh.py` — ETL + Benchmarks + Retreino ML
3. `pipeline/run_pipeline.py` — Orquestrador com CLI (--full, --etl-only, --download-only, --dry-run, --status)
4. `pipeline/crontab_config.sh` — Script de instalação/remoção do cron
5. Sistema de logs com histórico de execuções

---

## 4. Roadmap Futuro — Sprint 11
...
## 5. Visão de Produto — Maturidade

```
FASE 1 (Concluída)         FASE 2 (Concluída)         FASE 3 (Sprint 11+)
━━━━━━━━━━━━━━━━━━━        ━━━━━━━━━━━━━━━━━━━        ━━━━━━━━━━━━━━━━━━━
MVP Analítico              Motor Inteligente           Produto Comercial
• Dados reais ANS          • Predição ML ✅            • Interface dedicada
• 943 operadoras ✅        • MCP Server ✅             • Multi-tenant
• Dashboard v3.0 ✅        • Atualização automática ✅  • SaaS-ready
• Score de risco           • Retreino contínuo ✅      • Relatórios PDF
• Benchmark real ✅        • Pipeline cron ✅          • Mapas geográficos
• Série temporal 24T ✅    • 12 tools IA ✅
```

## 6. Dependências entre Sprints

```
Sprint 8 ✅ ──→ Sprint 8.1 ✅ ──→ Sprint 8.5 ✅ ──→ Sprint 9 ✅ (MCP) ──→ Sprint 11 (Frontend)
                                                       │
                                                       ▼
                                               Sprint 10 ✅ (Pipeline)
```
...
- **Sprint 9** ✅ — MCP Server (12 tools para agentes IA)
- **Sprint 10** ✅ — Pipeline automático (cron + ETL + retreino)
- **Sprint 11** 🔜 — Interface React (próxima)
...
| MCP Server (agentes IA consultam em linguagem natural) | ❌ | ❌ | ✅ Sprint 9 ✅ |
| Pipeline automático (cron + ETL + retreino) | ❌ | ❌ | ✅ Sprint 10 ✅ |
| Interface multi-tenant comercial | ❌ | ❌ | ✅ Sprint 11 |

---

*Documento Técnico — Motor de Sinistralidade ANS — Tallent Two Financial Holding*
*Autor: Ricardo Squillaci | Versão v2.4 | 21 de Maio de 2026*
